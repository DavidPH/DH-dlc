 ...
 SourcePosition const &pos, char const *fmt, ...)
{
   ...
   if (name)
      whatlen = snprintf
      (NULL, 0, "%s:%li:%li (%s:%i %s::%s): ", pos.file.c_str(), pos.line,
       pos.column, file, line, name, func);
   else
      whatlen = snprintf
      (NULL, 0, "%s:%li:%li (%s:%i %s): ", pos.file.c_str(), pos.line,
   ...
   if (name)
      whatprt = snprintf
      (whatstr, whatlen, "%s:%li:%li (%s:%i %s::%s): ", pos.file.c_str(), pos.line,
       pos.column, file, line, name, func);
   else
      whatprt = snprintf
      (whatstr, whatlen, "%s:%li:%li (%s:%i %s): ", pos.file.c_str(), pos.line,
       pos.column, file, line, func);
